...
"""
    ...
    for line in lines:
        parts = line.split(";")
        imei = parts[0].strip()
        add_info = parts[1].strip() if len(parts) > 1 and parts[1].strip() else f"MotoGuard {imei}"
...
def parse_obe_devices_data(doc_root, target_imeis: set = None):
    ...
    выбирает наиболее актуальную запись (приоритет: статус 0, свежая дата).
    """
    grouped = {}
    for d in doc_root.findall(".//{*}OBEDevicesData"):
        dev_id = (d.findtext("{*}GPSDeviceID") or "").strip()
        if not dev_id or (target_imeis and dev_id not in target_imeis):
            continue
...
def query_existing_devices_info(missing_imeis: set, max_wait_sec: int = 15):
    """
    ...
    for imei in missing_imeis:
        ...
        while time.time() - start_t < max_wait_sec:
            ...
            try:
                ...
                parsed = parse_obe_devices_data(doc_root, {imei})
                if imei in parsed and str(parsed[imei].get("status", "")).strip() == "0":
                    found_existing[imei] = parsed[imei]
                    break
            except Exception:
                pass
...
def wait_and_fetch_target_response(target_imeis: set, response_xml_path: Path, max_wait_sec: int = 25, poll_interval: int = 3):
    ...
    while time.time() - start_time < max_wait_sec:
        while True:
            ...
            try:
                ...
                parsed_123 = parse_obe_devices_data(doc_root, target_imeis)
                found_devices.update(parsed_123)
    ...
    if check_imeis:
        ...
        existing_info = query_existing_devices_info(check_imeis)
...
FLESPI_GROUP_MOTOGUARD_ETOLL = 247707  # "MotoGuard e-toll"


def sync_motoguard_flespi(imei: str):
    """Создает устройство в Flespi и добавляет в группу MotoGuard e-toll."""
    ...
    if not dev_id:
        try:
            payload = [{"name": f"Motoguard {clean_imei}", "device_type_id": 22, "messages_ttl": 1209600, "configuration": {"ident": clean_imei, "settings_polling": "once"}}]
    ...
    if dev_id:
        try:
            req = urllib.request.Request(f"{FLESPI_BASE_URL}/groups/{FLESPI_GROUP_MOTOGUARD_ETOLL}/devices/{dev_id}", headers=headers, method="POST")
            with urllib.request.urlopen(req, timeout=10, context=get_ssl_context()) as resp:
                return True, dev_id
        except Exception:
            pass
...
def save_results_documents(found_devices: dict, txt_path: Path, csv_path: Path):
    ...
    with open(csv_path, "w", encoding="utf-8") as f:
        ...
        for imei, d in found_devices.items():
            f.write(f"{d['deviceId']};{d['locNum']};{d['locPin']};{d['status']};{d['created']};{d['info']};MotoGuard e-toll\n")
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        ...
        f.write("-" * 110 + "\n")
        for imei, d in found_devices.items():
            f.write(f"{d['deviceId']:<18} | {d['locNum']:<20} | {d['locPin']:<6} | {d['status']:<8} | {'MotoGuard e-toll':<24} | {d['created']}\n")
        f.write("=" * 110 + "\n")
...
def main():
    ...
    if found_devices:
        ...
        # Синхронизация с группой Flespi (MotoGuard e-toll)
        print(f"\n📡 Синхронизация устройств с группой Flespi 'MotoGuard e-toll'...")
        for imei in found_devices.keys():
            ok, dev_id = sync_motoguard_flespi(imei)
            status_tag = "✅ добавлено в группу 'MotoGuard e-toll'" if ok else "⚠ не удалось добавить"
