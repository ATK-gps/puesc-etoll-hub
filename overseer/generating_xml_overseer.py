...
def query_existing_devices(missing_imeis: set, max_wait_sec: int = 15):
    """
    Отправляет ZSL_122 запрос для каждого устройства, которое не вернулось в ZSL_121.
    """
    found_existing = {}
    for imei in missing_imeis:
        print(f"🔎 Проверка статуса в базе PUESC для IMEI {imei}...")
        zsl122_xml = create_zsl122_query_xml(imei)
        success, sys_ref, _ = send_document_to_puesc(zsl122_xml, "ZSL_122.xml")
        if not success:
            continue
